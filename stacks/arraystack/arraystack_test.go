...
func TestStackSerialization(t *testing.T) {
	...
	var err error
	assert := func() {
		if actualValue, expectedValue := fmt.Sprintf("%s%s%s", stack.Values()[0], stack.Values()[1], stack.Values()[2]), "cba"; actualValue != expectedValue {
			t.Errorf("Got %v expected %v", actualValue, expectedValue)
		}
...
func benchmarkPop[T comparable](b *testing.B, stack *Stack[T], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			stack.Pop()
		}
