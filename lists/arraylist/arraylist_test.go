...
func TestOneMockRes(t *testing.T) {
	...
	//}
	//结合gorm、sqlmock
	DBMock, err := gorm.Open(mysql.Open("testtest:WgAewQMs3idC4aGnjGtBXw==@tcp(w_db.service.consul:3306)/tybdata?charset=utf8&loc=Local"), &gorm.Config{})
	if nil != err {
		log.Fatalf("Init DB with sqlmock failed, err %v", err)
	}

	var list ListEx[ActiveMemberApply]

	err = DBMock.Table("active_member_apply").Where("id = ?", 6679592).Find(&list).Error
	if err != nil {
		log.Fatalf("Init DB with sqlmock failed, err %v", err)
	}
...
//	}
//
//	if actualValue, ok := list2.Get(1); actualValue != 1 || !ok {
//		t.Errorf("Got %v expected %v", actualValue, "1")
//	}
//
//	if actualValue, ok := list2.Get(2); actualValue != nil || ok {
