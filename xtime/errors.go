...
var invalidValueError = func(value string) error {
	return fmt.Errorf("cannot parse %q as xtime, please make sure the value is valid", value)
}

// invalidLayoutError returns an invalid layout error.
// 无效的布局模板错误
var invalidLayoutError = func(value, layout string) error {
	return fmt.Errorf("cannot parse %q as xtime by layout %q, please make sure the value and layout match", value, layout)
}

// invalidFormatError returns an invalid format error.
// 无效的格式化字符错误
var invalidFormatError = func(value, format string) error {
	return fmt.Errorf("cannot parse %q as xtime by format %q, please make sure the value and format match", value, format)
}
