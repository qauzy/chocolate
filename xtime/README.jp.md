...
Time は [awesome-go](https://github.com/avelino/awesome-go#date-and-time "awesome-go") に収録されています, よかったら, スターをください
...
// ミリ秒のタイムスタンプから Time オブジェクトを作成します
...
// マイクロ秒タイムスタンプから Time オブジェクトを作成します
...
// 年月日から Time オブジェクトを作成します(時分秒はデフォルトで現在の時分秒です)
...
// 時分秒から Time オブジェクトを作成します(年月日のデフォルトは現在の年月日です)
...
##### 文字をフォーマットして文字列を Time オブジェクトに解析します
...
##### レイアウト文字を使用して文字列を Time オブジェクトに解析します
...
```go
// time.Time を Time に変換します
carbon.Time2Time(time.Now())
// Time を time.Time に変換します
